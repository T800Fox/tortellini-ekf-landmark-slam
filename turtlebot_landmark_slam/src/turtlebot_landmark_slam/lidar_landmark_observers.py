...
class lidarCylinderObserver:
    def __init__(self, 
        ...
        assert type(show_display) == bool 
        self.showDisplay = show_display

        self.maxLandmarks = max_landmark_count
        ...
        self.max_landmark_dist = max_landmark_dist # ignore landmarks over (m)
    ...
    def attemptAssociation(self, rel_points):
        ...
        close_points = [[point[0], point[1]] for point in rel_points 
                       if self._euclidianDistance(0,0, point[0], point[1]) < self.max_landmark_dist]
        ...
        for d in detections:
            ...
            if landmark_match != None:
                if landmark_match.id != -1:
                    print("landmark mean has shape -> ", landmark_match.mean[0:2].shape)
                    print("flattened landmark mean has shape -> ", landmark_match.mean[0:2].flatten().shape)
                    print("landmark mean -> \n", landmark_match.mean[0:2])

                    # ignore theta component
                    relativeXY, _, _  = Absolute2RelativeXY(current_pose, 
                                                            landmark_match.mean[0:2].flatten())
                    ...
                    measurement = LandmarkMeasurement(
                        ...
                        label=landmark_match.id,
                        is_new=is_new_landmark
                    )

                    landmark_measurements.append(measurement)


                    # landmark_measurements.append({'id':landmark_match.id,
                    #                     'x':float(relativeXY[0,0]),
                    #                     'y':float(relativeXY[1,0]),
    ...
    def _updateshow_display(self, rel_points, rel_detections):
