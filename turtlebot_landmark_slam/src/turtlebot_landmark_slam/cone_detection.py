...
def morphological(frame: cv2.Mat) -> cv2.Mat:
...
def cone_detection_and_extraction(frame: cv2.Mat, org_img: cv2.Mat, minContourRatio: float) -> list:
...
def red_mask(frame: cv2.Mat) -> cv2.Mat:
...
def green_mask(frame: cv2.Mat) -> cv2.Mat:
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
...
def yellow_mask(frame: cv2.Mat) -> cv2.Mat:
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
...
def blue_mask(frame: cv2.Mat) -> cv2.Mat:
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
...
def cone_detect_pipeline(
    ...
    # Morphological cleanup
    r_mask = morphological(r_mask)
    g_mask = morphological(g_mask)
    y_mask = morphological(y_mask)
    b_mask = morphological(b_mask)
    
    # Optional combined mask
    mask = r_mask | g_mask | y_mask | b_mask

    # ---- Cone extraction ----
    r_boxes, r_warped_images = cone_detection_and_extraction(r_mask, img, min_contour_ratio)
    
    g_boxes, g_warped_images = cone_detection_and_extraction(g_mask, img, min_contour_ratio)
    
    y_boxes, y_warped_images = cone_detection_and_extraction(y_mask, img, min_contour_ratio)
    
    b_boxes, b_warped_images = cone_detection_and_extraction(b_mask, img, min_contour_ratio)
    
    
    # Remove heavily overlapping boxes (e.g. from reflections or multiple contours on same cone)
    r_boxes, r_warped_images = remove_heavily_overlapping_detections(r_boxes, r_warped_images, 0.9)

    g_boxes, g_warped_images = remove_heavily_overlapping_detections(g_boxes, g_warped_images, 0.9)

    y_boxes, y_warped_images = remove_heavily_overlapping_detections(y_boxes, y_warped_images, 0.9)

    b_boxes, b_warped_images = remove_heavily_overlapping_detections(b_boxes, b_warped_images, 0.9)
